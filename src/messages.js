 ...
 * يعالج JID بشكل صحيح: 96597001234:8@s.whatsapp.net → 96597001234
 */
function currencyForJid(jid) {
  const num = String(jid || '')
    .replace(/@\w+\.\w+$/, '')   // إزالة @s.whatsapp.net أو @c.us أو @lid
    .split(':')[0]               // إزالة device suffix مثل :8
    .replace(/\D/g, '');         // الإبقاء على الأرقام فقط

  // DEBUG: يظهر في Railway logs لتشخيص مشكلة العملة
  console.log(`[currencyForJid] jid="${jid}" → num="${num}"`);
...
const messages = {
  ar: {
    welcome: `${BRAND_EMOJI} *أهلاً وسهلاً في ${BRAND}!*\n\n` +
    ...
    orderStatus: (order) => {
      ...
      if (order.fulfillment_status === 'unfulfilled') {
        const eta = order.estimated_delivery_at || order.shipping_lines?.[0]?.estimated_delivery_at || null;
        if (eta) {
  ...
  en: {
    welcome: `${BRAND_EMOJI} *Welcome to ${BRAND}!*\n\n` +
